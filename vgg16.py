...
Original file is located at
    https://colab.research.google.com/drive/1x_x2AVtF_6yVmJw6oVwp9-LGjXbOeGbn
"""
...
class ModelMaker:
    ...
    def _make_vgg_input_model(self, x):
        for layer_info in self.vgg_layers:
            copy_layer = layer_info["type"].from_config(layer_info["config"]) 
            x = copy_layer(x)  
            copy_layer.set_weights(layer_info["weights"])
        return x

    def _make_stage0(self, x):#VGG19
...
class ModelDatasetComponent:
    ...
    def place_training_labels(self, elem):
        ...
        if self.INCLUDE_MASK:
            ...
            inputs = image
        return inputs, (paf_tr, paf_tr, paf_tr, paf_tr, kpt_tr, kpt_tr)